...
mlp_model=mlp()
state_dict=torch.load("./new_checkpoints/best_DTC_single_GPU.pth", map_location=torch.device('cpu'))
...
pretrain_detector = init_detector(pretrain_config_file, pretrain_checkpoint_file, device='cpu')
# detector
...
detector = init_detector(config_file, checkpoint_file, device='cpu')
...
cmr_detector = init_detector(cmr_config_file, cmr_checkpoint_file, device='cpu')

class_num_in_detector=1
# feature model
config_file = './configs/efficientnet-b0_8xb32-01norm_in1k.py'
checkpoint_file='./new_checkpoints/feature.pth'
feature_model = init_model(config_file, checkpoint_file, device='cpu')
