...
class opts:
    def __init__(self):
        ...
        self.parser.add_argument(
            '--root_dataset',
            default='/data/ll/mot/data'
        )
        self.parser.add_argument(
            '--path_AFLink',
            default='/data/ll/mot/StrongSORT/data/AFLink_epoch20.pth'
        )
        self.parser.add_argument(
            '--dir_save',
            default='/data/ll/mot/StrongSORT/data/results/StrongSORT_Git/tmp'
        )
        self.parser.add_argument(
            '--EMA_alpha',
            default=0.9
    ...
    def parse(self, args=''):
        ...
        if opt.BoT:
            ...
            opt.dir_dets = '/data/ll/mot/StrongSORT/data/results/StrongSORT_Git/{}_{}_YOLOX+BoT'.format(opt.dataset, opt.mode)
        else:
            opt.max_cosine_distance = 0.3
            opt.dir_dets = '/data/ll/mot/StrongSORT/data/results/StrongSORT_Git/{}_{}_YOLOX+simpleCNN'.format(opt.dataset, opt.mode)
        if opt.MC:
        ...
        if opt.EMA:
            ...
            opt.nn_budget = 100
        if opt.ECC:
            path_ECC = '/data/ll/mot/StrongSORT/data/results/StrongSORT_Git/{}_ECC_{}.json'.format(opt.dataset, opt.mode)
